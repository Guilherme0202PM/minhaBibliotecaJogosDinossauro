...
public class RedeNeuralTeste2 {
        ...
        for (RedeNeuralTeste2 rede : populacao) {
            if (random.nextDouble() < taxaMutacaoPopulacional) {
                ...
                for (int i = 0; i < numOcultos1; i++) {
                    for (int j = 0; j < numSaidas; j++) {
                        double variacao = random.nextGaussian() * taxaMutacaoIndividual;
                        rede.pesosOcultaSaida1[i][j] += variacao;
                    }
                }
    ...
    public void aplicarMutacaoPopulacional2(List<RedeNeuralTeste2> populacao) {
        Random random = new Random();

        // Aplica mutação populacional em 50% dos indivíduos
        for (RedeNeuralTeste2 rede : populacao) {
            if (random.nextDouble() < taxaMutacaoPopulacional) {
                // Aplica mutação individual de 30% em cada peso da primeira camada
                for (int i = 0; i < numEntradas; i++) {
                    for (int j = 0; j < numOcultos1; j++) {
                        double variacao = random.nextGaussian() * taxaMutacaoIndividual;
                        rede.pesosEntradaOculta1[i][j] += variacao;
                ...
                // Mutação nos pesos da camada de saída (para rede com uma camada oculta)
                for (int i = 0; i < numOcultos1; i++) {
                    for (int j = 0; j < numSaidas; j++) {
                        double variacao = random.nextGaussian() * taxaMutacaoIndividual;
                        rede.pesosOcultaSaida1[i][j] += variacao;
